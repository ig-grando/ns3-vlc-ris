...
ns3::TypeId VlcMobilityModel::GetTypeId(void) {
	static ns3::TypeId tid = ns3::TypeId("ns3::VlcMobilityModel").SetParent<
			MobilityModel>().SetGroupName("Mobility").AddConstructor<
			VlcMobilityModel>().AddAttribute("Azimuth",
...
inline Vector VlcMobilityModel::DoGetVelocity(void) const {
...
inline Vector VlcMobilityModel::DoGetPosition(void) const {
	double t = (Simulator::Now() - m_baseTime).GetSeconds();
	double half_t_square = t * t * 0.5;
	return Vector(
			m_basePosition.x + m_baseVelocity.x * t
			...
			m_basePosition.z + m_baseVelocity.z * t
					+ m_acceleration.z * half_t_square); // returns a acceleration vector of the node that has this mobility model instlled on it
}

void VlcMobilityModel::DoSetPosition(const Vector &position) //called to set position via vector parameters
		{
...
void VlcMobilityModel::SetVelocityAndAcceleration(const Vector &velocity,
